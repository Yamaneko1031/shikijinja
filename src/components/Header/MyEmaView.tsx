...
import EmaSeat from '../Ema/EmaSeat';
import Image from 'next/image';

type Props = {
  emaPostResponses: EmaPostResponse[];
  onClose: () => void;
};

export default function MyEmaView({ emaPostResponses, onClose }: Props) {
  ...
  return (
    <div className="select-none">
      <div className="min-h-[100lvh] pt-5 flex flex-col items-center">
        ...
        >
          <EmaSeat emaPostResponse={emaPostResponses[currentIndex]} />
        </div>
      </div>
      <div className="fixed w-full bottom-6 m-auto flex flex-row justify-center gap-2">
        ...
        <div className="w-12 text-md flex items-center justify-center drop-shadow-[1px_1px_1px_rgba(0,0,0,1)]">
          {currentIndex + 1} / {emaPostResponses.length}
        </div>
        <Button
          variant="subNatural"
          size="md"
          onClick={() => setCurrentIndex(currentIndex + 1)}
          disabled={currentIndex === emaPostResponses.length - 1}
          aria-label="次"
        >
          次
        </Button>
